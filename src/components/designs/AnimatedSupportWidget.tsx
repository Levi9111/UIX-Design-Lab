...
function useWidgetState() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (!isExpanded) setPulseKey((prev) => prev + 1);
    }, 3000);
  ...
  return { isExpanded, isVisible, pulseKey, toggleExpanded, handleWhatsApp, handleTelegram };
}

// ─── Desktop variant ─────────────────────────────────────────────────────────

const ForDesktop = () => {
  const { isExpanded, isVisible, pulseKey, toggleExpanded, handleWhatsApp, handleTelegram } =
    useWidgetState();
  ...
  return (
    <div className='fixed bottom-6 right-6 z-50 animate-widget-float'>
      <div
        ...
        >
          <div className='relative flex items-center justify-center'>
            <div
              ...
              rounded-full transition-all duration-700 ${isExpanded ? 'animate-spin' : 'animate-pulse'}`}
            />
            <div className='relative z-10 group-hover:scale-110 transition-transform duration-300'>
              <div className={`transition-all duration-300 ${isExpanded ? 'rotate-90' : 'rotate-0'}`}>
                {isExpanded ? (
...
const ForMobile = () => {
  const { isExpanded, isVisible, pulseKey, toggleExpanded, handleWhatsApp, handleTelegram } =
    useWidgetState();
  const [activeButton, setActiveButton] = useState<'whatsapp' | 'telegram' | null>(null);
  ...
  return (
    <div className='fixed bottom-4 right-4 z-50 animate-widget-float-mobile'>
      <div
        className={`relative bg-gradient-to-br from-slate-900/95 via-slate-800/95 to-slate-900/95
        backdrop-blur-xl border border-white/25 rounded-2xl shadow-2xl transition-all duration-500 ease-out
      ...
      >
        ...
        <div className='absolute inset-0 overflow-hidden rounded-2xl'>
          <div
            ...
            rounded-2xl blur-lg transition-all duration-500 ${
        ...
        >
          <div className='relative flex items-center justify-center'>
            <div
              ...
              rounded-full transition-all duration-300 ${isExpanded ? 'animate-spin' : 'animate-pulse'}`}
            />
